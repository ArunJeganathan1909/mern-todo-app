...
const Planned = () => {
  ...
  const plannedTasks = todos.filter((todo) =>
    todo.categories.includes("planned")
  );
  ...
  return (
    <div className="display">
      ...
      {plannedTasks.length === 0 ? (
        ...
        plannedTasks.map((todo, index) => (
          <div key={todo._id} className="display-task">
            <div className="date">
              {index === 0 ||
              new Date(todo.date).toDateString() !==
                new Date(plannedTasks[index - 1].date).toDateString() ? (
